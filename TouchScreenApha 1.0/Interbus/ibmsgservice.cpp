...
void IBMsgService::slotRxMessageTimeout()
{
    ...
    if (wasRunning)
    {
        ...
        emit sigReceiverStatus(rxDeviceId, rxRegisterId, txMsgType, 0xFF, QByteArray());
    }
    else
...
void IBMsgService::slotSendMessage(quint8 devId, quint8 regId, quint8 msgType, QByteArray msgData)
{
...
void IBMsgService::slotResendMessage()
{
    ...
    //qDebug() << "Resend" << txMasterId << txDeviceId << txMsgType << txRegisterId;
                ...
                if (rxMsg.size() >= 5)   // Dest + Src + Type + msbCRC + lsbCRC = Minimum telegram length.
                {
                    if (rxCalcCRC == 0)
                    {
                        if (((quint8)rxMsg.data()[0]) == txMasterId && ((quint8)rxMsg.data()[1]) == txDeviceId)
                        {
                            ...
                            emit sigReceiverStatus(rxDeviceId, rxRegisterId, txMsgType, rxMsg.data()[2], QByteArray(rxMsg.mid(4, rxMsg.size()-6)));
                        }
                        else
                        {
                            //showMessage("RX:", rxMsg);
