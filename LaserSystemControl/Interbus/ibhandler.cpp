...
IBHandler::IBHandler(QObject *parent) : QObject(parent)
{
    ...
    if (QMetaType::type("RegWriteTypes") == QMetaType::UnknownType)
        qRegisterMetaType<RegWriteTypes>( "RegWriteTypes" );
    if (QMetaType::type("PriorityTypes") == QMetaType::UnknownType)
        qRegisterMetaType<PriorityTypes>( "PriorityTypes" );
    if (QMetaType::type("RegDataTypes") == QMetaType::UnknownType)
        qRegisterMetaType<RegDataTypes>( "RegDataTypes" );
    ...
    connect(m_msgService, SIGNAL(sigReceiverStatus(quint8,quint8,quint8,quint8,QByteArray)), this, SLOT(slotReceiverStatus(quint8,quint8,quint8,quint8,QByteArray)));
...
RegSetStatusTypes IBHandler::writeRegister(const quint8 devId, const quint8 regId, const QByteArray &writeData)
{
    ...
    if (m_serialport->isOpen())
    {
        ...
        m_txCRCRetries = 3;
        m_txBUSYRetries = 3;
        m_msgService->slotSendMessage(devId, regId, 0x05, writeData);
...
RegGetStatusTypes IBHandler::readRegister(const quint8 devId, const quint8 regId, QByteArray &readData)
{
    ...
    if (m_serialport->isOpen())
    {
        ...
        m_txCRCRetries = 3;
        m_txBUSYRetries = 3;
        m_msgService->slotSendMessage(devId, regId, 0x04, QByteArray());
...
void IBHandler::slotReceiverStatus(quint8 deviceId, quint8 registerId, quint8 txMsgType, quint8 rxMsgType, QByteArray msgData)
{
    Q_UNUSED(deviceId)
    Q_UNUSED(registerId)
    ...
    switch (rxMsgType)
    {
    case 0x01:  // CRC error - Remote module has reported an CRC error - Resend
        if (m_txCRCRetries > 0)
        {
            m_txCRCRetries--;
    ...
    case 0x02:  // Busy - Remote module has send a Busy - Resend
    ...
    case 0xFF:  // Communication error - The IBMsgService has not received a complete message within the timeout period - Resend if allowed
        if (m_txMsgRetries > 0)
        {
            m_txMsgRetries--;
    ...
    case 0x00:  // Nack - Remote module has send a Nack - Do not resend since the message won't be accepted
    case 0x03:  // Ack - Remote module has send an Ack (Successfull write)
    case 0x08:  // Datagram - Remote module has send a Datagram (Successfull read)
    default:
        passRxMessage(txMsgType, rxMsgType, msgData);
    }
}
...
void IBHandler::passRxMessage(quint8 txMsgType, quint8 rxMsgType, QByteArray msgData)
{
    if (txMsgType == 0x04)      // Send, was a read command
    {
        m_rxMsgData = msgData;
        switch (rxMsgType)
        {
    ...
    else if (txMsgType == 0x05 || txMsgType == 0x06 || txMsgType == 0x07 || txMsgType == 0x09) // Send was a write command
    {
        switch (rxMsgType)
        {
        case 0x00:  // Nack
        ...
        case 0x02:  // Busy
            m_txStatus = SetBusy;
            break;
        case 0x03:  // Ack - (Successfull write)
